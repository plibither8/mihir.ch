...
<div class="space-y-5" style="min-width: 25rem">
  {#each data.slice(0, loadCount) as { id, link, time, text, type, title }, i}
    <article class="space-y-2 relative">
      <span class="absolute top-0 right-full text-gray-500 mr-3"
        >{data.length - i}.</span
      >
      {#if type === 'story'}
        <a
          class="text-gray-300 link"
          href={link}
          target="_blank"
          rel="noopener"
        >
      ...
      <p class="text-sm text-gray-400">
        ...
        <a
          class="link"
          href="https://news.ycombinator.com/id?={id}"
          target="_blank"
          rel="noopener"
        >
          show on hn
  ...
  {#if loadCount < data.length}
    <button
      class="text-gray-300 flex items-center space-x-3"
      on:click={() => (loadCount += 5)}
    >
